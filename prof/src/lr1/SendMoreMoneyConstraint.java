...
public class SendMoreMoneyConstraint extends Constraint <Character, Integer> { // //Определим ограничения посредством класса Constraint

    private List<Character> letters;

    //мы определяем Constraint  как абстрактный базовый класс.
    public SendMoreMoneyConstraint(List<Character> letters) {
        super(letters);
        this.letters = letters;
    }

    //метод satisfied()  проверяет выполняется ли ограничение
    @Override
